		...
		<script type="text/javascript">
			...
			function cucsupdata() {
				...
				if(bbsrow>0){ //有資料再刷新
					...
					var t_where = "where=^^ 1=1 and ("+new_where+") and isnull(a.gen,0)=0 and isnull(b.hours,0)=0 and isnull(b.mount2,0)>0 order by b.size,b.spec,b.lengthb desc,b.noa,b.noq ^^";
					var t_where1 = "where[1]=^^ cb.productno2=b.noa and cb.product2=b.noq and case when isnull(ca.itype,'')='' then '1' else ca.itype end ='3' ^^";
			...
			function q_gfPost() {
				...
				//載入案號 資料
                var t_where = "where=^^ 1=1 and isnull(a.gen,0)=0 and isnull(b.hours,0)=0 and isnull(b.mount2,0)>0 order by b.size,b.spec,b.lengthb desc,b.noa,b.noq ^^";
                var t_where1 = "where[1]=^^ cb.productno2=b.noa and cb.product2=b.noq and case when isnull(ca.itype,'')='' then '1' else ca.itype end ='3' ^^";
				...
				q_cmbParse("combMechno2",'4@車牙,7@7辦公室');
                ...
                $('#btnImport').click(function(e) {
                	...
                	if(t_cucno.length>0){
	                    ...
	                    var t_where = " 1=1 and isnull(a.gen,0)=0 and isnull(b.hours,0)=0 and isnull(b.mount2,0)>0 ";
				...
				string+='<td id="cuct_uno" align="center" style="width:210px; color:black;">批號</td>';
				...
				$('#btnPlut').click(function() {
					...
					$('#cuct_table input[type="text"]').each(function() {
						...
						//只能輸入數值
						if(objname=='textGmount' || objname=='textGweight' || objname=='textGlengthc'){
						...
						if(objname=='textUno'){
							$(this).change(function() {
								if($(this).val()!=''){
									//找批號
									q_func('qtxt.query.cuctgetuno_'+n, 'cuc_vu.txt,getuno,'+$(this).val()+';#non'+';#non'+';#non');
								}else{
									$('#btnMinut_'+n).click();
								}
							});
				...
				string+='<td id="cuct_uno" align="center" style="width:210px; color:black;">批號&nbsp;&nbsp;<input type="button" id="btnCub_nouno" value="領料" style="width:60px;font-size: medium; font-weight: bold"/></td>';
				string+='<td id="cuct_product" align="center" style="width:120px; color:black;">品名</td>';
				string+='<td id="cuct_ucolor" align="center" style="width:170px; color:black;">類別</td>';
				string+='<td id="cuct_spec" align="center" style="width:140px; color:black;">材質</td>';
				string+='<td id="cuct_size" align="center" style="width:90px; color:black;">號數</td>';
				...
				$('#btnCub_nouno').click(function(e) {
					...
					for(var j=0;j<bbtrow;j++){
						if($('#textUno_'+j).val().length>0){
							...
							q_func('qtxt.query.cuctcheckuno_'+n, 'cuc_vu.txt,getuno,'+$('#textUno_'+j).val()+';#non'+';#non'+';#non');
						}
					}
					...
					if(t_err.length>0){
						...
						alert('無入庫資料');
	                   	...
	                   	if(confirm("確認要入庫?")){
							...
							+r_accy+';'+t_datea+';'+t_mechno+';'+t_memo+';'+r_userno.toUpperCase()+';'+r_name+';'+ts_bbu+';3');
						}
					}
				});
            ...
            function q_gtPost(t_name) {
				switch (t_name) {
					case 'init':
						...
						string+='<td id="cucs_scolor" title="續接品名" align="center" style="width:80px; color:black;">續接品名</td>';
						string+='<td id="cucs_paraf" title="續接規格1" align="center" style="width:40px; color:black;">續接<BR>規格1</td>';
						string+='<td id="cucs_parag" title="續接規格2" align="center" style="width:40px; color:black;">續接<BR>規格2</td>';
						string+='<td id="cucs_img" title="圖形" align="center" style="width:103px; color:black;" class="imghide">圖形</td>';
						...
						string+='<td id="cucs_btime" title="顏色1" align="center" style="width:28px; color:black;display:none;">顏色<BR>1</td>';
						string+='<td id="cucs_etime" title="顏色2" align="center" style="width:28px; color:black;display:none;">顏色<BR>2</td>';
						string+='<td id="cucs_memo" title="備註(標籤)" align="center" style="width:83px; color:black;">備註(標籤)</td>';
						string+='<td id="cucs_4mount" title="彎" align="center" style="width:28px; color:black;display:none;">彎</td>';
						string+='<td id="cucs_2mount" title="車" align="center" style="width:28px; color:black;display:none;">車</td>';
						...
						string+='<td id="cucs_work" title="工令" align="center" style="width:78px; color:black;display:none;">工令</td>';
						...
						string+="<td id='cucs_hours' align='center' style='width:28px; color:black;'><input type='button' id='btnHours' style='font-size:12px;width: 30px;height: 45px;white-space: inherit;' value='已車牙'/></td>";
						string+='<td id="cucs_dime" title="待成型" align="center" style="width:22px; color:black;">待成型</td>';
						string+='</tr>';
						string+='</table>';
						$('#cucs').html(string);
						...
						string+='<td id="cucs_no2" title="訂單序號" align="center" style="width:90px; color:black;display:none;">訂單序號</td>';
						//string+='<td id="cucs_mins" align="center" style="width:30px; color:black;">完工</td>';
						string+="<td id='cucs_hours' align='center' style='width:28px; color:black;'><input type='button' id='btnHours2' style='font-size:12px;width: 30px;height: 45px;white-space: inherit;' value='已車牙'/></td>";
						string+='<td id="cucs_dime" title="待成型" align="center" style="width:22px; color:black;">待成型</td>';
						string+='</tr>';
						string+='</table>';
						$('#cucs_float').remove();
					...
					case 'importcucs':
							...
							for(var j=0;j<bbsrow;j++){
								...
								for(var i =0 ;i<as.length;i++){
									if(as[i].noa==$('#cucs_noa'+j).text() && as[i].noq==$('#cucs_noq'+j).text()){
										...
										$('#cucs_scolor'+j).text(as[i].scolor);
						...
						for(var i=0;i<as.length;i++){
							if(table_noa!='' && table_noa!=as[i].noa){ //不同案號 空依格
								...
								string+='<td id="cucs_scolor'+(i+bbsrow)+'" style="text-align: center;color:'+t_color[(i+bbsrow)%t_color.length]+'"></td>';
								...
								string+='<td id="cucs_img'+(i+bbsrow)+'" style="text-align: center;color:'+t_color[(i+bbsrow)%t_color.length]+'" class="imghide"><img id="Imgcucs_img'+(i+bbsrow)+'" style="width:100px;"></td>';
								...
								string+='<td id="cucs_btime'+(i+bbsrow)+'" style="display:none;text-align: center;color:'+t_color[(i+bbsrow)%t_color.length]+'"><a id="lblCucs_btime'+(i+bbsrow)+'"></a></td>';
								string+='<td id="cucs_etime'+(i+bbsrow)+'" style="display:none;text-align: center;color:'+t_color[(i+bbsrow)%t_color.length]+'"><a id="lblCucs_etime'+(i+bbsrow)+'"></a></td>';
								string+='<td id="cucs_memo'+(i+bbsrow)+'" style="text-align: center;color:'+t_color[(i+bbsrow)%t_color.length]+'"></td>';
								string+='<td id="cucs_4mount'+(i+bbsrow)+'" style="display:none;text-align: center;color:'+t_color[(i+bbsrow)%t_color.length]+'"></td>';
								string+='<td id="cucs_2mount'+(i+bbsrow)+'" style="display:none;text-align: center;color:'+t_color[(i+bbsrow)%t_color.length]+'"></td>';
								...
								string+='<td id="cucs_work'+(i+bbsrow)+'" style="display:none;text-align: center;color:'+t_color[(i+bbsrow)%t_color.length]+'"></td>';
								...
								string+='<td style="text-align: center;"></td>';
								string+='<td id="cucs_dime'+(i+bbsrow)+'" style="text-align: center;color:'+t_color[(i+bbsrow)%t_color.length]+'"></td>';
								string+='</tr>';
								bbsrow++;
							}
							...
							string+='<td id="cucs_scolor'+(i+bbsrow)+'" style="text-align: center;color:'+t_color[(i+bbsrow)%t_color.length]+'">'+as[i].scolor+'</td>';
							...
							string+='<td id="cucs_img'+(i+bbsrow)+'" style="text-align: center;color:'+t_color[(i+bbsrow)%t_color.length]+'" class="imghide"><img id="Imgcucs_img'+(i+bbsrow)+'" src="'+as[i].imgbarcode+'" style="width:100px;"></td>';
							string+='<td id="cucs_class'+(i+bbsrow)+'" style="text-align: center;color:'+t_color[(i+bbsrow)%t_color.length]+'"><a id="lblCucs_class'+(i+bbsrow)+'">'+as[i].class+'</a></td>';
							...
							string+='<td id="cucs_btime'+(i+bbsrow)+'" style="display:none;text-align: center;color:'+t_color[(i+bbsrow)%t_color.length]+'"><a id="lblCucs_btime'+(i+bbsrow)+'">'+as[i].btime+'</a><select id="combXbtime_'+(i+bbsrow)+'" class="txt comb" style="width: 20px;"> </select></td>';
							string+='<td id="cucs_etime'+(i+bbsrow)+'" style="display:none;text-align: center;color:'+t_color[(i+bbsrow)%t_color.length]+'"><a id="lblCucs_etime'+(i+bbsrow)+'">'+as[i].etime+'</a><select id="combXetime_'+(i+bbsrow)+'" class="txt comb" style="width: 20px;"> </select></td>';
							string+='<td id="cucs_memo'+(i+bbsrow)+'" style="text-align: center;color:'+t_color[(i+bbsrow)%t_color.length]+'">'+as[i].memo+'</td>';
							string+='<td id="cucs_4mount'+(i+bbsrow)+'" style="display:none;text-align: center;color:'+t_color[(i+bbsrow)%t_color.length]+'">'+(dec(as[i].mount4)>0?'V':'')+'</td>';
							string+='<td id="cucs_2mount'+(i+bbsrow)+'" style="display:none;text-align: center;color:'+t_color[(i+bbsrow)%t_color.length]+'">'+(dec(as[i].mount2)>0?dec(as[i].mount2):'')+'</td>';
							...
							string+='<td id="cucs_work'+(i+bbsrow)+'" style="display:none;text-align: center;color:'+t_color[(i+bbsrow)%t_color.length]+'">'+as[i].size2+'</td>';
							...
							string+='<td id="cucs_dime'+(i+bbsrow)+'" style="text-align: center;color:'+t_color[(i+bbsrow)%t_color.length]+'">'+(dec(as[i].dime)>0 ?'V':'')+'</td>';
							string+='</tr>';
						...
						$('#cucs .cucs_chk').click(function(e) {
							...
							//Lock();
							var t_where="where=^^  1=1 and a.noa='"+$('#cucs_noa'+n).text()+"' and b.noq='"+$('#cucs_noq'+n).text()+"' and isnull(a.gen,0)=0 and isnull(b.hours,0)=0 and isnull(b.mount2,0)>0 ^^";
							var t_where1 = "where[1]=^^ cb.productno2=b.noa and cb.product2=b.noq and case when isnull(ca.itype,'')='' then '1' else ca.itype end ='3' ^^";
							//判斷是否能被鎖定或解除
							if($(this).prop('checked')){
						...
						if(imp_cucno.length>0){
							for(var i=0;i<bbsrow;i++){
								...
								&& !$('#cucs_chk'+i).prop('disabled')){ //沒有被核取過的資料 且目前沒被鎖定過
									...
									var t_where="where=^^ 1=1 and a.noa='"+$('#cucs_noa'+i).text()+"' and b.noq='"+$('#cucs_noq'+i).text()+"' and isnull(a.gen,0)=0 and isnull(b.hours,0)=0 and isnull(b.mount2,0)>0 ^^";
									var t_where1 = "where[1]=^^ cb.productno2=b.noa and cb.product2=b.noq and case when isnull(ca.itype,'')='' then '1' else ca.itype end ='3' ^^";
                    	...
                    	if (as[0] != undefined){
                    		...
                    		if(t_err.length>0){
								...
								if(stkupdate==0){
	                    			...
	                    			+r_userno.toUpperCase()+';'+r_name+';'+t_noa+';'+t_noq+';'+t_xmount+';'+t_xcount+';'+t_xweight+';'+ts_bbt+';3');
	                    			//取消刷新
	                    			//clearInterval(intervalupdate);
								}
							}
                ...
                if(t_func.indexOf('qtxt.query.cuctcheckuno_')>-1){
                	...
                	if(getunocount==0){ //批號檢查完畢
                		if(getunoerr.length>0){
							...
							if(t_nouno.length>0){
								q_func('qtxt.query.cubnouno', 'cuc_vu.txt,cubnouno_vu,' + encodeURI(r_accy) + ';' + encodeURI(t_nouno)+ ';' + encodeURI(t_mechno)+ ';' + encodeURI(r_userno)+ ';' + encodeURI(r_name)+';3');
								Lock();
							}
                		}
                	}
