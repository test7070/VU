		...
		<script type="text/javascript">
			...
			function cucsupdata() {
				...
				if(bbsrow>0){ //有資料再刷新
					...
					var t_where1 = "where[1]=^^ cb.productno2=b.noa and cb.product2=b.noq and case when isnull(ca.itype,'')='' then '1' else ca.itype end ='2' ^^";
			...
			function q_gfPost() {
				...
				q_cmbParse("combMechno2",'5@成型,7@7辦公室');
					...
					if(t_err.length>0){
						...
						alert('無入庫資料');
	                   	...
	                   	if(confirm("確認要入庫?")){
							...
							+r_accy+';'+t_datea+';'+t_mechno+';'+t_memo+';'+r_userno.toUpperCase()+';'+r_name+';'+ts_bbu+';2');
						}
					}
				});
            ...
            function q_gtPost(t_name) {
				switch (t_name) {
					case 'init':
						...
						string+='<td id="cucs_btime" title="顏色1" align="center" style="display:none;width:28px; color:black;">顏色<BR>1</td>';
						string+='<td id="cucs_etime" title="顏色2" align="center" style="display:none;width:28px; color:black;">顏色<BR>2</td>';
						string+='<td id="cucs_memo" title="備註(標籤)" align="center" style="width:83px; color:black;">備註(標籤)</td>';
						string+='<td id="cucs_4mount" title="彎" align="center" style="display:none;width:28px; color:black;">彎</td>';
						string+='<td id="cucs_2mount" title="車" align="center" style="display:none;width:28px; color:black;">車</td>';
						...
						string+="<td id='cucs_waste' align='center' style='width:28px; color:black;'><input type='button' id='btnWaste' style='font-size:12px;width: 30px;height: 45px;white-space: inherit;' value='已成型'/></td>";
						string+='</tr>';
						string+='</table>';
						$('#cucs').html(string);
						...
						string+='<td id="cucs_ordeno" title="訂單號碼" align="center" style="width:90px; color:black;display:none;">訂單號碼</td>';
						string+='<td id="cucs_no2" title="訂單序號" align="center" style="width:90px; color:black;display:none;">訂單序號</td>';
						string+="<td id='cucs_waste' align='center' style='width:28px; color:black;'><input type='button' id='btnWaste2' style='font-size:12px;width: 30px;height: 45px;white-space: inherit;' value='已成型'/></td>";
						string+='</tr>';
						string+='</table>';
						$('#cucs_float').remove();
						$('#cucs').append(string);
                    	...
                    	if (as[0] != undefined){
                    		...
                    		if(t_err.length>0){
								...
								if(stkupdate==0){
	                    			...
	                    			+r_userno.toUpperCase()+';'+r_name+';'+t_noa+';'+t_noq+';'+t_xmount+';'+t_xcount+';'+t_xweight+';'+ts_bbt+';2');
	                    			//取消刷新
	                    			//clearInterval(intervalupdate);
								}
							}
